...
class NetworkDiffusion(NetworkModel): 
    def f(self, t, u, args):
        transport_rate = args 
        du = -transport_rate * jnp.dot(self.L, u) 
        return du
...
class NetworkFKPP(NetworkModel): 
    def f(self, t, u, args):
        ...
        du = -transport_rate * jnp.dot(self.L, u) + production_rate * u * ( 1 - u )
        return du

class ScaledNetworkFKPP(NetworkModel):
    def __init__(self, c, u0, ui):
        self.u0 = u0
        self.ui = ui
    ...
    def f(self, t, u, args):
        transport_rate, production_rate = args
        du = -transport_rate * jnp.dot(self.L, u - self.u0) + production_rate * (u - self.u0) * ( (self.ui - self.u0) - (u - self.u0) )
        return du
...
class NetworkATN(NetworkModel):
    def f(self, t, u, args):
        ...
        d_y = -tau_transport_rate * jnp.dot(self.L, y) + tau_production_rate * y * ( (baseline + coupling * x ) - y )
        d_z = atrophy_rate * y * ( 1 - z )
        Du = jnp.vstack([d_x, d_y, d_z])
        return Du
    
class ScaledNetworkATN(NetworkModel):
    def __init__(self, c, ui, part): 
        self.ui = ui
        self.part = part
    ...
    def f(self, t, u, args):
        ...
        d_y = -tau_transport_rate * jnp.dot(self.L, y) + tau_production_rate * (self.part + (coupling * x)) * y * ( 1 - y )
        d_z = atrophy_rate * y * ( 1 - z )
        Du = jnp.vstack([d_x, d_y, d_z])
        return Du
